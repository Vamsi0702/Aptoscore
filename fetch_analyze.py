...
def train_and_predict(transaction_df):
    ...
    # Handle cases where there are very few transactions
    if len(transaction_df) < 5:
        print("Not enough transaction data to train the model. Using default reputation score = 50.")
        transaction_df["reputation_score"] = 50  # Assign default score
        return transaction_df

    # Extract features and labels
    X = transaction_df[["gas_used", "gas_price", "success"]]
    y = np.random.randint(50, 100, len(X))  # Fake labels (since we don’t have real reputation scores yet)
...
if __name__ == "__main__":
    transactions = fetch_transactions(ACCOUNT_ADDRESS, limit=50000)
